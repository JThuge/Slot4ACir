...
def collate_fn(batch):
    '''
    function which discards None images in a batch when using torch DataLoader
...
class FashionIQDataset(Dataset):
    ...
    def __init__(self, split: str, dress_types: List[str], mode: str, preprocess: callable, llm_sample=False):
        """
        ...
        if llm_sample and split == 'train':
            for dress_type in dress_types:
                with open(base_path / 'FashionIQ' / 'captions' / f'cap.{dress_type}.llm.{split}.json') as f:
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                if self.llm_sample:
                    img1 = random.sample(image_captions[0:3], k=1)[0]
                    img2 = random.sample(image_captions[3:], k=1)[0]
                    image_captions = [img1, img2]
                reference_name = self.triplets[index]['candidate']

                if self.split == 'train':
                    reference_image_path = base_path / 'FashionIQ' / 'images' / f"{reference_name}.png"
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                    target_name = self.triplets[index]['target']
                    target_image_path = base_path / 'FashionIQ' / 'images' / f"{target_name}.png"
                ...
                image_path = base_path / 'FashionIQ' / 'images' / f"{image_name}.png"
...
class CIRRDataset(Dataset):
    ...
    """
    def __init__(self, split: str, mode: str, preprocess: callable, llm_sample=False, textual_inversion=False, sub=False):
        """
        :param split: dataset split, should be in ['test', 'train', 'val']
        :param mode: dataset mode, should be in ['relative', 'classic']:
                  - In 'classic' mode the dataset yield tuples made of (image_name, image)
        ...
        self.llm_sample = llm_sample
        if llm_sample and split == 'train':
            with open(base_path / 'CIRR' / 'cirr' / 'captions' / f'cap.rc2.llm.{split}.json') as f:
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                if self.llm_sample and self.split == 'train':
                    rel_caption = random.sample(rel_caption, k=1)[0]
                    
                if self.textual_inversion:
                    rel_caption = f'a photo of $ that ' + rel_caption.lower()
