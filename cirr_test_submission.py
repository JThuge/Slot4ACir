...
def generate_cirr_test_submissions(file_name: str, blip_model, preprocess, txt_processors, rerank, exp_name, mode='test1'):
    """
   ...
   """
    ...
    classic_test_dataset = CIRRDataset(mode, 'classic', preprocess)
    index_features, index_names = extract_index_blip_features(classic_test_dataset, blip_model)
    relative_test_dataset = CIRRDataset(mode, 'relative', preprocess, sub=True)
    ...
    submissions_folder_path = Path(f"./submission/CIRR/{exp_name}")
