...
def backup_code(source_dir, log_dir):
    ...
    for root, dirs, files in os.walk(source_dir):
        if os.path.basename(root) in ['output', '__pycache__', 'misc', 'submission', 'EfficientViT', 'mobilenet']:
            dirs[:] = []
            continue
        for file in files:
        ...
        for dir in dirs:
            if dir in ['output', '__pycache__', 'misc', 'submission', 'EfficientViT', 'mobilenet']:
                continue
...
def extract_index_blip_features(dataset: Union[CIRRDataset, FashionIQDataset], blip_model, save_memory=False) -> \
    ...
    if isinstance(dataset, CIRRDataset):
        ...
        raise ValueError("Not implemented yet: {}".format(dataset.__repr__))
    for names, images in tqdm(classic_val_loader):
        images = images.to(device, non_blocking=True)
        with torch.no_grad():
...
def generate_randomized_fiq_caption(flattened_captions: List[str]) -> List[str]:
    ...
    for i in range(0, len(flattened_captions), 2):
        ...
        if random_num < 0.25:
            captions.append(
                f"{prefix}{flattened_captions[i].strip('.?, ').lower()} and {flattened_captions[i + 1].strip('.?, ').lower()}")
        elif 0.25 < random_num < 0.5:
            captions.append(
                f"{prefix}{flattened_captions[i + 1].strip('.?, ').lower()} and {flattened_captions[i].strip('.?, ').lower()}")
        elif 0.5 < random_num < 0.75:
            captions.append(f"{prefix}{flattened_captions[i].strip('.?, ').lower()}")
        else:
            captions.append(f"{prefix}{flattened_captions[i + 1].strip('.?, ').lower()}")
    return captions
...
def save_model(name: str, cur_epoch: int, model_to_save: nn.Module, training_path: Path, mode='epoch'):
    """
    ...
    if mode == 'epoch':
        ...
        raise NotImplementedError(f'The save mode {mode} has not been implemented.')
